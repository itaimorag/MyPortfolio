...
function cellClicked(elCell, iIdx, jIdx) {
    ...
    if (elCell.isMarked) return
        ...
        if (gFirstMoveMegaHint === 0) {
            ...
            showCellsMegaHint(gFirstMove.i, gFirstMove.j, iIdx, jIdx)
        setTimeout(hideCellsMegaHint, 2000, gFirstMove.i, gFirstMove.j, iIdx, jIdx)
    }
    else if (gIsHintOn) {
...
function hideCellsMegaHint(previousI, previousJ, currentI, currentJ) {
    for (var i = previousI; i <= currentI; i++) {
        for (var j = previousJ; j <= currentJ; j++) {
            if (gBoard[i][j].isShown) {
                gBoard[i][j].isShown = false
